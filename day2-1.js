...
inputstream.split("\n").forEach(e => {
    if (e.match(regexRed).filter(e1 => parseInt(e1) > 12).length > 0)
        return;
    if (e.match(regexGreen).filter(e1 => parseInt(e1) > 13).length > 0)
        return;
    if (e.match(regexBlue).filter(e1 => parseInt(e1) > 14).length > 0)
        return;
    sum += parseInt(e.match(regexGame));
});
...
//takeaways:
//do NOT have an empty line on the text file or else have
//if (e.match(regexRed) != null)